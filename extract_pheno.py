...
def extract_phenotypes(ID_list, output_fname, combine=False, combine_op="last", chunksize=10000, pheno_dir=None, target_dir=".", debug=False):
    ...
    # file_list = sorted(glob(pheno_dir), key=lambda x: stat(x).st_size) # <-- sort by smallest file size to largest
    if debug:
...
if __name__ == "__main__":
    ...
    extract_phenotypes(ID_list, output_fname, combine, combine_op, chunksize, pheno_dir, target_dir, debug)

    # test commands (reads ukb*.csv in test directory)
    # python3 extract_pheno.py 21 31 34 44 45 50 72 -n comb -c -r 100 -d tests -t dummy -v
    # python3 extract_pheno.py 31-0.0 34-0.0 44-0.0 50-0.0 50-1.0 50-2.0 -n nocom -r 100 -d tests -t dummy -v
